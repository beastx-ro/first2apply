...
alter table public.notes enable row level security;
alter table public.advanced_filters enable row level security;
...
create policy "enable insert advanced filters for authenticated users only" 
on public.advanced_filters 
as permissive 
for insert 
to authenticated 
with check (auth.uid() = user_id);

create policy "enable select advanced filters for authenticated users only" 
on public.advanced_filters 
as permissive 
for select 
to authenticated 
...
BEGIN
    -- Delete existing filters for the current user
    DELETE FROM advanced_filters WHERE user_id = current_user_id;

    -- Insert new filters or update existing ones
    INSERT INTO advanced_filters (user_id, "filterName", rules)
    SELECT current_user_id, filter->>'filterName', (filter->>'rules')::jsonb
    FROM jsonb_array_elements(new_filters) AS filter
    ON CONFLICT (id) DO UPDATE
    SET "filterName" = EXCLUDED."filterName",
