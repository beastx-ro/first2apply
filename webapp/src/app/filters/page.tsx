...
import { StripeBillingPlan, SubscriptionTier } from "@/lib/supabase/types";
...
export default function FiltersPage() {
  ...
  const [addBlacklistedCompany, setAddBlacklistedCompany] =
    useState<string>("");
  const [isSubscriptionDialogOpen, setSubscriptionDialogOpen] =
    useState<boolean>(false);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [showAllBlacklistedCompanies, setShowAllBlacklistedCompanies] =
    useState(false);
  ...
  useEffect(() => {
    const asyncLoad = async () => {
      try {
        ...
        handleError({
          error,
          title: "Failed to load advanced matching filters",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="flex flex-col space-y-16 p-6 md:p-10">
      <h1 className="w-fit text-2xl font-medium tracking-wide">
        Advanced Matching
      </h1>

      <section>
        <p className="mb-4 text-lg">
          Set your preferences and let <span className="font-medium">AI</span>{" "}
          find the <span className="font-medium">right jobs</span> for you. Just
        ...
        <Alert className="flex items-center gap-2 border-0 p-0">
          ...
          <AlertDescription className="text-sm text-muted-foreground">
            Pro Tips: Exclude skills you don’t want, specify salary
            expectations, define experience levels, select job specifics like
            remote work or PTO preferences and more.
          </AlertDescription>
        </Alert>
      </section>
      ...
      <section>
        <p className="mb-4 text-lg">
          <span className="font-medium">Blacklist companies</span> you don’t
          want to work for. We’ll make sure you{" "}
        ...
        <Alert className="mt-1.5 flex items-center gap-2 border-0 p-0">
          ...
          <AlertDescription className="text-sm text-destructive/90">
            Attention: Ensure you input the company name accurately without any
            typos.
          </AlertDescription>
        </Alert>
          ...
          {blacklistedCompanies.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {(showAllBlacklistedCompanies
                ? blacklistedCompanies
                : blacklistedCompanies.slice(0, 10)
              ).map((company) => (
                <Badge
                  key={company}
                    ...
                    <Tooltip>
                      <TooltipTrigger>
                        <Cross2Icon
                          ...
                          onClick={() =>
                            setBlacklistedCompanies(
                              blacklistedCompanies.filter((c) => c !== company)
                            )
                          }
                        />
                      </TooltipTrigger>
              ...
              {blacklistedCompanies.length > 10 &&
                !showAllBlacklistedCompanies && (
                  <Button
                    variant="secondary"
                    className="py-2"
                    onClick={() => setShowAllBlacklistedCompanies(true)}
                  >
                    See All
                  </Button>
                )}
              {showAllBlacklistedCompanies && (
                <Button
                  variant="secondary"
                  className="py-2"
                  onClick={() => setShowAllBlacklistedCompanies(false)}
                >
                  Show Less
                </Button>
              )}
